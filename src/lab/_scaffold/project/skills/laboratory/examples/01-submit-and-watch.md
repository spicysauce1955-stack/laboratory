...
- For long jobs pass `--timeout` to `lab wait` so you eventually get woken even
  if the job hangs. It accepts a **duration string or bare seconds** —
  `--timeout 2h` and `--timeout 7200` are the same deadline.

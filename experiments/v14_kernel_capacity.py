"""V14 -- KERNEL-PARAMETERIZED tempotron capacity alpha_c(K_eff) across PSP kernel families.

V14 = V13 + three opt-in, surgical campaign additions (default behavior is byte-identical to V13):

1. **Direct physical-width override** ``width_list=`` (aliases ``tau_list=``/``sigma_list=``/``omega0_list=``).
   When present the K_eff bisection is BYPASSED: each given physical width becomes one cell (the
   "fix the knob, vary the grid" / rect dt-ladder mode), and the certified axes (Keff_rice, D_PR,
   kappa_S, lambda_*) are still computed and logged per width. ``Keff_list=`` still works unchanged
   when no width override is given. A ``width`` column is logged on every row.

2. **Restart-overlap witness q0** (``capture_overlap=1``, default 0 -> no cost). After the
   ``n_restarts`` independent solved-weight vectors are found for a cell, q0 is the mean pairwise
   normalized overlap ``mean_{r<r'} |w_r . w_r'| / (||w_r|| ||w_r'||)`` over the restarts that SOLVED
   (label-flip-invariant via ``|.|``; NaN if <2 solved restarts). Mirrors the v9 restart-overlap
   logic; the solution-cluster / shattering observable for ln K vs ln ln K growth and the radial
   existence-vs-findable gap. With capture on, the restart loop runs ALL restarts (no early break).

3. **Spectrally band-limited alpha kernel ``alpha_trunc``** (the I4 test): the causal alpha with its
   powerless high-frequency tail stripped by an ideal low-pass at ``trunc_omega`` (argv, rad/ms,
   default ``8/tau``). Built deterministically/seedlessly via rFFT on an internal fine lag grid
   (zero bins with omega>trunc_omega, irFFT, renormalize peak to 1) and evaluated by linear
   interpolation at the query lags. By construction it holds the low-frequency power of ``alpha`` but
   carries a ~8% lower ``Keff_rice`` at matched ``tau`` while leaving E[max V] / capacity nearly
   unchanged.
...
Run standalone (tiny smoke; does NOT need a GPU):
    LAB_RUN_DIR=/tmp/v14_smoke uv run --with torch python studies/v14_kernel_capacity.py \\
        kernel=alpha_trunc width_list=20 N_list=30 alphas=1.0,2.0 seeds=0,1 epochs=80 \\
        oversample=4 capture_overlap=1
"""
...
def _alpha_trunc_eval(s: torch.Tensor, tau: float, trunc_omega: float) -> torch.Tensor:
    """Spectrally band-limited causal alpha: alpha low-passed at ``trunc_omega`` [rad/ms].

    Deterministic and seedless. The causal alpha ``(t/tau) e^{1 - t/tau}`` is built on an INTERNAL
    fine, uniform lag grid (step resolves several * trunc_omega so the cutoff is well below Nyquist),
    rFFT'd, all bins with ``omega > trunc_omega`` are zeroed (ideal low-pass), irFFT'd, and the peak
    is renormalized to 1. The resulting band-limited kernel is then sampled at the (arbitrary-shaped)
    query lags ``s`` by linear interpolation -- so it can stand in for ``eval_kernel`` at the
    non-uniform lags used by ``precompute_traces`` while remaining a genuinely band-limited PSP.

    The point (the I4 test): stripping the powerless omega>trunc_omega tail of alpha lowers the Rice
    crossing axis ``Keff_rice = T sqrt(lambda_2/lambda_0)`` (lambda_2 weights by omega^2) by ~8% at
    ``trunc_omega = 8/tau`` while leaving the low-frequency power -- hence E[max V] / capacity -- nearly
    unchanged.
    """
    dev = s.device
    trunc = trunc_omega if trunc_omega > 0.0 else 8.0 / max(tau, 1e-9)
    # Internal grid: step resolves the cutoff with headroom (Nyquist >> trunc); extent covers the
    # alpha support AND the query lags (so interpolation never extrapolates inside the band).
    dt_i = min(math.pi / (8.0 * max(trunc, 1e-9)), tau / 16.0)
    smax = float(s.detach().abs().max().item()) if s.numel() else 16.0 * tau
    L = max(smax, 16.0 * tau) + 8.0 * tau
    ng = int(2.0 * L / dt_i) + 1
    half = ng // 2
    idx = torch.arange(ng, device=dev, dtype=torch.float64) - half
    grid = idx * dt_i                                   # symmetric uniform lags
    sc = torch.clamp(grid, min=0.0)
    raw = (sc / tau) * torch.exp(1.0 - sc / tau)
    raw = torch.where(grid >= 0.0, raw, torch.zeros_like(raw))
    Khat = torch.fft.rfft(raw)
    f = torch.fft.rfftfreq(ng, d=dt_i).to(dev)          # cycles/ms
    omega = 2.0 * math.pi * f                           # rad/ms
    Khat = torch.where(omega > trunc, torch.zeros_like(Khat), Khat)
    filt = torch.fft.irfft(Khat, n=ng)
    peak = filt.max()
    if float(peak) != 0.0:
        filt = filt / peak                              # renormalize peak to 1
    # Linear interpolation of the band-limited kernel at the query lags (uniform grid -> O(1) index).
    grid0 = grid[0]
    pos = (s.to(torch.float64) - grid0) / dt_i
    posc = pos.clamp(min=0.0, max=float(ng - 1))
    i0 = posc.floor().to(torch.long)
    i1 = (i0 + 1).clamp(max=ng - 1)
    frac = posc - i0.to(torch.float64)
    vals = filt[i0] * (1.0 - frac) + filt[i1] * frac
    inside = (pos >= 0.0) & (pos <= float(ng - 1))
    out = torch.where(inside, vals, torch.zeros_like(vals))
...
def eval_kernel(s: torch.Tensor, kernel: str, params: dict) -> torch.Tensor:
    ...
    if kernel == "alpha_trunc":
        # Spectrally band-limited causal alpha (ideal low-pass at trunc_omega); the I4 test kernel.
        tau = params["tau"]
        trunc_omega = params.get("trunc_omega", 0.0)
        return _alpha_trunc_eval(s, tau, trunc_omega)
    if kernel == "rect":
...
_KNOB_INCREASING = {"sinc": True, "sinusoid": True, "gauss": False,
                    "gabor": False, "alpha": False, "alpha_trunc": False, "rect": False}
...
def _axis_for_width(kernel: str, width: float, omega0: float, T: float, oversample: float,
                    n_lobes: int, window: str, trunc_omega: float = 0.0) -> dict:
    ...
    ``trunc_omega`` carries the ``alpha_trunc`` low-pass cutoff (0 -> auto 8/tau); ignored otherwise.
    """
    params = _params_for(kernel, width, omega0, n_lobes, window, trunc_omega)
...
def _params_for(kernel: str, width: float, omega0: float, n_lobes: int, window: str,
                trunc_omega: float = 0.0) -> dict:
    p = {"n_lobes": n_lobes, "window": window, "omega0": omega0, "n_lobes_win": n_lobes}
    p[_WIDTH_KEY[kernel]] = width
    if kernel == "alpha_trunc":
        p["trunc_omega"] = trunc_omega if trunc_omega > 0.0 else 8.0 / max(width, 1e-9)
    return p
...
def _coarse_bandwidth(kernel: str, width: float, omega0: float) -> float:
    ...
    if kernel == "alpha_trunc":
        return 1.0 / max(width, 1e-9)      # 1/tau (the alpha band; the trunc tail is powerless)
    if kernel == "rect":
...
def invert_knob_to_keff(kernel: str, target: float, *, T: float, oversample: float,
                        n_lobes: int, window: str, omega0: float, trunc_omega: float = 0.0,
    ...
    def measured(width: float) -> float:
        return _axis_for_width(kernel, width, omega0, T, oversample, n_lobes, window, trunc_omega)[axis]
    ...
    if not (math.isfinite(fa) and math.isfinite(fb)):
        ...
        if idx.size == 0:
            ...
            width = float(grid[j])
            return _axis_for_width(kernel, width, omega0, T, oversample, n_lobes, window, trunc_omega) | {
...
def main() -> int:
    ...
    kernel = ov.get("kernel", "sinc")            # sinc|gauss|gabor|alpha|alpha_trunc|rect|sinusoid
    keff_list = [float(x) for x in ov.get("Keff_list", "2,8,32").split(",")]
    # (1) Direct physical-width override: BYPASS the K_eff bisection, one cell per given width.
    # Aliases tau_list/sigma_list/omega0_list are accepted for readability (all map to the width knob).
    width_override: list[float] | None = None
    for _wk in ("width_list", "tau_list", "sigma_list", "omega0_list"):
        if _wk in ov:
            width_override = [float(x) for x in ov[_wk].split(",") if x != ""]
            break
    ...
    # (2) Restart-overlap witness q0 (opt-in; runs ALL restarts so the solution cluster is sampled).
    capture_overlap = int(ov.get("capture_overlap", "0"))
    # (3) alpha_trunc low-pass cutoff [rad/ms]; 0 -> auto 8/tau per cell.
    trunc_omega = float(ov.get("trunc_omega", "0"))
    ...
    cell_values = width_override if width_override is not None else keff_list
    cell_mode = "width" if width_override is not None else "Keff"
    print(
        f"V14 kernel capacity | kernel={kernel} axis={axis} cell_mode={cell_mode} seed={master_seed} "
        f"device={device} cells={cell_values} N={n_list} alphas={alphas} seeds={seeds} epochs={epochs} "
        f"lr={lr} mom={momentum} T={T_ms} oversample={oversample} n_lobes={n_lobes} window={window} "
        f"density_coeff={density_coeff} restarts={n_restarts} capture_overlap={capture_overlap} "
        f"trunc_omega={trunc_omega} mode={mode} optimizer={optimizer} "
    ...
    n_cells = len(cell_values) * len(n_list) * len(alphas)
    cell_i = 0
    ...
    for keff_t in cell_values:
        if width_override is not None:
            # ---- (1) WIDTH OVERRIDE: bypass the bisection; the cell value IS the physical width. ----
            # Still compute & log the certified axes (Keff_rice, D_PR, kappa_S, lambda_*) for this width;
            # control falls through to the shared inner loop (density keyed off the measured Keff_rice).
            width = keff_t
            inv = _axis_for_width(kernel, width, gabor_omega0, T_ms, oversample,
                                  n_lobes, window, trunc_omega) | {"width": width}
            params = inv["params"]
            Q = gabor_omega0 * width if kernel == "gabor" else float("nan")
        else:
            ...
            inv = invert_knob_to_keff(kernel, keff_t, T=T_ms, oversample=oversample,
                                       n_lobes=n_lobes, window=window, omega0=gabor_omega0,
                                       trunc_omega=trunc_omega, axis=axis)
            width = inv["width"]
            params = inv["params"]
            Q = float("nan")
            if kernel == "gabor":
        ...
        if mean_spikes_fixed > 0:
            ...
            # Width-override: there is no target K_eff; key the pattern density off the measured Rice
            # axis (or D_PR when Keff_rice is non-finite, e.g. rect) so density tracks expressiveness.
            base = Keff_rice if math.isfinite(Keff_rice) and Keff_rice > 0 else D_PR
            mean_spikes = max(1e-3, density_coeff * base)
        else:
            mean_spikes = max(1e-3, density_coeff * keff_t)

        if width_override is not None:
            print(f"  [width] kernel={kernel} {_WIDTH_KEY[kernel]}={width:.5g} "
                  f"| Keff_rice={Keff_rice:.3f} D_PR={D_PR:.3f} kappa_S={kappa_S:.3f} Q={Q} "
                  f"dt={dt:.4g} n_grid={n_grid} mean_spikes={mean_spikes:.3g}", flush=True)
        else:
        ...
        for n_aff in n_list:
            for alpha in alphas:
                ...
                overlap_acc: list[float] = []  # (2) pairwise |cos| over solved restarts (q0 witness)
                ...
                for b0 in range(0, n_seeds, sb_size):
                    ...
                    thr = res["threshold"]
                    # (2) Restart-overlap witness: keep each restart's converged-weight vector so the
                    # solution cluster can be sampled. Only allocated when capture_overlap (no cost off).
                    restart_ws: list[tuple[torch.Tensor, torch.Tensor]] = []
                    if capture_overlap:
                        restart_ws.append((res["converged"].clone(), res["weights"].clone()))
                    for r in range(1, n_restarts):
                        ...
                        if capture_overlap:
                            restart_ws.append((rr["converged"].clone(), rr["weights"].clone()))
                        # Default path keeps the v13 early-out; with capture_overlap we run ALL restarts
                        # so the full solution cluster (not just the first hit) is sampled for q0.
                        if bool(any_conv.all()) and not capture_overlap:
                            break
                    conv_multi[b0:b1] = any_conv
                    if capture_overlap and len(restart_ws) >= 2:
                        # q0 = mean_{r<r'} |w_r . w_r'| / (||w_r|| ||w_r'||) over SOLVED restarts, per seed.
                        convs = torch.stack([c for c, _ in restart_ws], dim=0)   # (R, nb)
                        wsr = torch.stack([w for _, w in restart_ws], dim=0)      # (R, nb, N)
                        nb = b1 - b0
                        for jj in range(nb):
                            sidx = torch.nonzero(convs[:, jj], as_tuple=False).flatten()
                            if sidx.numel() < 2:
                                continue
                            wj = wsr[sidx, jj]                                    # (m, N)
                            wn = wj / wj.norm(dim=1, keepdim=True).clamp_min(1e-12)
                            G = (wn @ wn.t()).abs()                               # (m, m) |cos|
                            m = wn.shape[0]
                            iu = torch.triu_indices(m, m, offset=1, device=G.device)
                            overlap_acc.extend(G[iu[0], iu[1]].detach().cpu().numpy().tolist())
                    del s
                    if device.type == "cuda":
                ...
                print(
                    ...
                    + (f" q0={restart_overlap:.3f}" if capture_overlap else ""),
                    flush=True,
                )
        if n_list:
    ...
    try:
        ...
        git_sha = None
    results = {
        "experiment": "v14_kernel_capacity",
        "kernel": kernel,
        "axis": axis,
        "cell_mode": cell_mode,
        ...
        "params": {
            ...
            "width_override": width_override, "capture_overlap": capture_overlap,
            "trunc_omega": trunc_omega,
        ...
        "per_keff": [per_keff[k] for k in cell_values if k in per_keff],
        "rows": rows,
    ...
    for k in cell_values:
        if k in per_keff:
            pk = per_keff[k]
            lab = f"{_WIDTH_KEY[kernel]}={k:.3g}" if width_override is not None else f"K_eff target={k:.1f}"
            print(f"  {kernel} {lab} measured={pk['Keff_measured']:.2f} "
